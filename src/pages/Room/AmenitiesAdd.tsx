...
const AmenitiesAdd: React.FC = () => {
      ...
      const validateForm = (data: FormData): ValidationErrors => {
            ...
            else if (data.name.length > 50) errors.name = 'Tên không được vượt quá 50 ký tự';
            if (data.description && data.description.length > 500) errors.description = 'Mô tả không được vượt quá 500 ký tự';
            if (data.icon && !isValidUrl(data.icon)) errors.icon = 'URL biểu tượng không hợp lệ';
            if (data.price < 0) errors.price = 'Giá không được âm';
            if (data.default_quantity < 0) errors.default_quantity = 'Số lượng mặc định không được âm';
            ...
            try {
                  ...
                  if (!response.ok) {
                        return response.text().then((text) => {
                              throw new Error(`Lỗi thêm tiện ích: ${response.status} ${response.statusText}. Chi tiết: ${text}`);
                        });
                  }
