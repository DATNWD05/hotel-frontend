...
import {
  ...
  Pagination, Dialog, DialogTitle, DialogContent, DialogActions, Chip,
...
const RoomTypesList: React.FC = () => {
  ...
  const [amenityQuantities, setAmenityQuantities] = useState<Record<number, number>>({});
  ...
  // ========== CHỈNH: Lưu tiện nghi kèm số lượng ==========
  const handleSave = async () => {
    ...
    try {
      ...
      // 2) Đồng bộ tiện nghi kèm quantity (CHÍNH LÝ DO MẤT SỐ LƯỢNG)
      ...
      // BE nên trả danh sách amenities kèm quantity sau khi sync
      const updatedFromBE: Amenity[] = (amenRes?.data?.data || []).map((a: any) => ({
        id: a.id,
        name: a.name,
      ...
      setRoomTypes(prev =>
        prev.map(rt =>
          rt.id === editFormData.id
            ? { ...editFormData, amenities: updatedFromBE.length ? updatedFromBE : rt.amenities }
            : rt
        )
      );
      ...
      toast.success('Cập nhật loại phòng & số lượng tiện nghi thành công!');
    } catch (err: unknown) {
      const errorMessage =
        err instanceof AxiosError
          ? err.response?.status === 401
  ...
  const handleQtyChange = (amenityId: number, value: number) => {
    setAmenityQuantities(prev => ({ ...prev, [amenityId]: Math.max(1, Math.floor(value || 1)) }));
  };

  const incQty = (amenityId: number) => {
    setAmenityQuantities(prev => ({ ...prev, [amenityId]: Math.max(1, (prev[amenityId] || 1) + 1) }));
  };
  ...
  const toggleAddAmenity = (amenityId: number, checked: boolean) => {
    ...
    if (checked) setAddQuantities(prev => ({ ...prev, [amenityId]: prev[amenityId] ?? 1 }));
  };

  const handleAddQtyChange = (amenityId: number, value: number) => {
    setAddQuantities(prev => ({ ...prev, [amenityId]: Math.max(1, Math.floor(value || 1)) }));
  };
  ...
  const handleAddAmenityConfirm = async () => {
    ...
    try {
      ...
      const updatedAmenities: AmenityPayload[] = Array.from(mergedMap.entries()).map(([id, quantity]) => ({ id, quantity }));
      ...
      if (response.status === 200) {
        ...
        updated.forEach(a => {
          newQtyMap[a.id] = Math.max(1, Number((a as any)?.quantity ?? (a as any)?.pivot?.quantity ?? 1));
        });
        ...
        setRoomTypes(prev =>
          prev.map(rt =>
            rt.id === editFormData.id
              ? { ...rt, amenities: updated.map(u => ({ id: u.id, name: u.name, quantity: newQtyMap[u.id] })) }
              : rt
          )
        );
  ...
  const QtyFieldInline = ({
    ...
    <Box display="flex" alignItems="center" gap={0.5}>
      <IconButton size="small" onClick={() => onChange(Math.max(1, (value || 1) - 1))}>
      ...
      <TextField
        ...
        size="small"
        value={value}
        onChange={(e: ChangeEvent<HTMLInputElement>) => onChange(Math.max(1, Number(e.target.value) || 1))}
  ...
  return (
    <div className="promotions-wrapper">
                      ...
                      <TableCell align="center">
                        <Box display="flex" justifyContent="center" gap={1} sx={{ flexWrap: 'wrap' }}>
                          <IconButton
                            ...
                            sx={{ color: '#1976d2', bgcolor: '#e3f2fd', p: '6px', '&:hover': { bgcolor: '#bbdefb', boxShadow: '0 2px 6px rgba(25, 118, 210, 0.4)' } }}
                          >
                          ...
                          <IconButton
                            ...
                            sx={{ color: '#FACC15', bgcolor: '#fef9c3', p: '6px', '&:hover': { bgcolor: '#fff9c4', boxShadow: '0 2px 6px rgba(250, 204, 21, 0.4)' } }}
                          >
                          ...
                          <IconButton
                            ...
                            sx={{ color: '#d32f2f', bgcolor: '#ffebee', p: '6px', '&:hover': { bgcolor: '#ffcdd2', boxShadow: '0 2px 6px rgba(211, 47, 47, 0.4)' } }}
                          >
                              ...
                              <Box sx={{ p: 2, bgcolor: '#fff', borderRadius: '8px' }}>
                                ...
                                <Box display="flex" flexDirection="column" gap={2}>
                                  <Box display="flex" gap={2}>
                                    <TextField label="Mã" name="code" value={editFormData.code} onChange={handleChange} fullWidth variant="outlined" size="small" error={!!validationErrors.code} helperText={validationErrors.code}/>
                                    <TextField label="Tên" name="name" value={editFormData.name} onChange={handleChange} fullWidth variant="outlined" size="small" error={!!validationErrors.name} helperText={validationErrors.name}/>
                                  </Box>
                                  <Box display="flex" gap={2}>
                                    <TextField label="Số người tối đa" name="max_occupancy" type="number" value={editFormData.max_occupancy} onChange={handleChange} fullWidth variant="outlined" size="small" error={!!validationErrors.max_occupancy} helperText={validationErrors.max_occupancy} inputProps={{ min: 0 }}/>
                                    <TextField label="Giá cơ bản" name="base_rate" type="number" value={editFormData.base_rate} onChange={handleChange} fullWidth variant="outlined" size="small" error={!!validationErrors.base_rate} helperText={validationErrors.base_rate} inputProps={{ min: 0 }} InputProps={{ endAdornment: <InputAdornment position="end">đ</InputAdornment> }}/>
                                  </Box>
                                  <TextField label="Mô tả" name="description" value={editFormData.description} onChange={handleChange} fullWidth variant="outlined" size="small" multiline rows={3} error={!!validationErrors.description} helperText={validationErrors.description}/>

                                  <Typography variant="h6" mt={1} mb={1} sx={{ fontWeight: 600, color: '#333' }}>
                                    Tiện nghi (kèm số lượng)
                                  </Typography>

                                  <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, bgcolor: '#fff', p: 2, borderRadius: '8px', border: '1px solid #eee' }}>
                                    {selectedAmenities.length > 0 ? (
                                      selectedAmenities.map(amenityId => {
                                        const amenity = allAmenities.find(a => a.id === amenityId);
                                        const qty = amenityQuantities[amenityId] ?? 1;
                                        if (!amenity) return null;
                                        return (
                                          <Box key={amenityId} sx={{ display: 'flex', alignItems: 'center', gap: 1, p: 1, borderRadius: '8px', border: '1px solid #E0E0E0', bgcolor: '#fafbff' }}>
                                            <Chip label={amenity.name} size="small" color="primary" variant="outlined" sx={{ borderColor: '#4318FF', color: '#4318FF' }}/>
                                            <QtyFieldInline amenityId={amenityId} value={qty} onChange={(val) => handleQtyChange(amenityId, val)} />
                                            <IconButton size="small" onClick={() => handleRemoveAmenity(amenityId)} title="Gỡ tiện nghi" sx={{ ml: 0.5 }}>
                                    ...
                                    <Button startIcon={<AddIcon />} color="primary" variant="outlined" onClick={handleAddAmenityClick} sx={{ ml: 1, borderStyle: 'dashed' }}>
                                      Thêm tiện nghi
                                    </Button>
                                  </Box>
                                  ...
                                  <Box mt={2} display="flex" gap={2}>
                                    <Button variant="contained" onClick={handleSave} disabled={editLoading}
                                      sx={{ backgroundColor: '#4318FF', color: '#fff', textTransform: 'none', fontWeight: 600, borderRadius: '8px', px: 2.5, py: 0.7, '&:hover': { backgroundColor: '#7B1FA2' }, '&:disabled': { backgroundColor: '#a9a9a9' } }}>
                                      {editLoading ? <CircularProgress size={24} /> : 'Lưu'}
                                    </Button>
                                    <Button variant="outlined" onClick={handleCancel} disabled={editLoading}
                                      sx={{ color: '#f44336', borderColor: '#f44336', textTransform: 'none', fontWeight: 600, borderRadius: '8px', px: 2.5, py: 0.7, '&:hover': { borderColor: '#d32f2f', backgroundColor: '#ffebee' }, '&:disabled': { color: '#a9a9a9', borderColor: '#a9a9a9' } }}>
                                      Hủy
                                    </Button>
                                  </Box>
                                  ...
                                  <Box display="flex" flexWrap="wrap" gap={1}>
                                    {rt.amenities.length > 0 ? (
                                      rt.amenities.map((amenity) => (
                                        <Chip
                                          key={amenity.id}
                                          label={typeof amenity.quantity === 'number' ? `${amenity.name} × ${amenity.quantity}` : amenity.name}
                                          size="small"
                                          color="primary"
                                          variant="outlined"
      ...
      {/* Dialog xóa */}
      <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)} sx={{ '& .MuiDialog-paper': { borderRadius: '8px', boxShadow: '0 2px 8px rgba(0, 0, 0, 0.15)' } }}>
        ...
        <DialogActions>
          <Button onClick={() => setDeleteDialogOpen(false)} sx={{ color: '#d32f2f', borderColor: '#d32f2f', textTransform: 'none', fontWeight: 600, borderRadius: '8px', px: 2.5, py: 0.7, '&:hover': { borderColor: '#b71c1c', backgroundColor: '#ffebee' } }}>
            Hủy
          </Button>
          <Button onClick={confirmDelete} variant="contained" sx={{ bgcolor: '#d32f2f', '&:hover': { bgcolor: '#b71c1c' }, textTransform: 'none', fontWeight: 600, borderRadius: '8px', px: 2.5, py: 0.7 }}>
            Xác nhận
          </Button>
        </DialogActions>
      </Dialog>
      ...
      {/* Dialog thêm tiện nghi */}
      <Dialog open={addAmenityDialogOpen} onClose={handleCancelAddAmenity} maxWidth="sm" fullWidth sx={{ '& .MuiDialog-paper': { borderRadius: '8px', boxShadow: '0 2px 8px rgba(0, 0, 0, 0.15)' } }}>
          ...
          <Box display="grid" gap={1}>
            {allAmenities
              ...
              .map(amenity => {
                ...
                return (
                  <Box key={amenity.id} sx={{ display: 'grid', gridTemplateColumns: '1fr auto', alignItems: 'center', gap: 1, p: 1, border: '1px solid #eee', borderRadius: '8px', bgcolor: checked ? '#fafbff' : '#fff' }}>
                    <FormControlLabel control={<Checkbox checked={checked} onChange={(e) => toggleAddAmenity(amenity.id, e.target.checked)} />} label={amenity.name}/>
                    <Box display="flex" alignItems="center" gap={0.5}>
                      <Typography variant="body2" color="text.secondary">SL:</Typography>
                      <TextField type="number" size="small" value={qty} onChange={(e) => handleAddQtyChange(amenity.id, Number(e.target.value))} inputProps={{ min: 1, style: { width: 80 } }} disabled={!checked}/>
                    </Box>
                  </Box>
                );
              })}
            {allAmenities.filter(a => !selectedAmenities.includes(a.id)).length === 0 && (
              <Typography variant="body2" color="text.secondary">Không còn tiện nghi để thêm.</Typography>
            )}
          </Box>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleCancelAddAmenity} sx={{ color: '#d32f2f', borderColor: '#d32f2f', textTransform: 'none', fontWeight: 600, borderRadius: '8px', px: 2.5, py: 0.7, '&:hover': { borderColor: '#b71c1c', backgroundColor: '#ffebee' } }}>
            Hủy
          </Button>
          <Button onClick={handleAddAmenityConfirm} variant="contained" sx={{ bgcolor: '#4318FF', '&:hover': { bgcolor: '#7B1FA2' }, textTransform: 'none', fontWeight: 600, borderRadius: '8px', px: 2.5, py: 0.7 }}>
            Thêm
          </Button>
        </DialogActions>
      </Dialog>
