...
const CreateService: React.FC = () => {
  ...
  const validateForm = (data: ServiceFormData): ValidationErrors => {
    ...
    else if (data.name.length > 255) errors.name = 'Tên dịch vụ không được vượt quá 255 ký tự';
    if (!data.category_id) errors.category_id = 'Vui lòng chọn danh mục dịch vụ';
    if (data.description && data.description.length > 1000) errors.description = 'Mô tả không được vượt quá 1000 ký tự';
    if (data.price < 0 || data.price > 9999999999) errors.price = 'Giá dịch vụ phải từ 0 đến 9,999,999,999';
    return errors;
  };
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      ...
      try {
        ...
        const res = await fetch('http://127.0.0.1:8000/api/service-categories', { headers });
        if (!res.ok) {
          if (res.status === 401) throw new Error('Không được phép truy cập. Vui lòng đăng nhập.');
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    const errors = validateForm({ ...formData, [name]: name === 'price' ? Number(value) : value });
      ...
      <div className="create-service-title">
        <div className="create-service-header-content">
          ...
          <Box className="create-service-form-buttons">
            ...
            <Button
              variant="outlined"
              className="create-service-btn-cancel"
              color="secondary"
        ...
        <div className="create-service-detail-container">
          ...
          <Box display="flex" flexDirection="column" gap={2}>
            <Box display="flex" gap={2}>
              ...
              />
              <FormControl fullWidth variant="outlined" size="small" error={!!validationErrors.category_id}>
