...
interface CheckoutInfo {
  ...
  is_hourly?: number; // 1 = theo giờ
  hours?: number; // số giờ, nếu BE có trả
}
...
const CheckoutDialog: React.FC<CheckoutDialogProps> = ({
  ...
  const changeRowRoom = (i: number, room_id: number) => {
    setAmenitiesUsed((prev) => {
      ...
      const qty = Math.min(
        clampPositiveInt(old?.quantity ?? 1),
  ...
  const changeAmenity = (i: number, opt: AmenityOption | null) => {
    setAmenitiesUsed((prev) => {
      ...
      if (remaining <= 0) {
        toast.error(
          `Tiện nghi "${opt.name}" cho phòng này đã đạt giới hạn (${maxQty}).`
        );
        return next;
      }
  ...
  const changeQty = (i: number, v: string) => {
    setAmenitiesUsed((prev) => {
      ...
      const usedElsewhere = getUsedElsewhere(
        next,
        i,
        row.room_id,
        row.amenity_id
  ...
  const sanitizeAmenities = (rows: AmenityRow[]) => {
    ...
    valid.forEach((r, idx) => {
      ...
      const usedElsewhere = valid.reduce((s, rr, ii) => {
        if (ii === idx) return s;
        if (rr.room_id === r.room_id && rr.amenity_id === r.amenity_id)
          return s + clampPositiveInt(rr.quantity);
        return s;
      }, 0);
      const remaining = Math.max(0, maxQty - usedElsewhere);
                      ...
                      <TableRow key={i} hover>
                        ...
                        <TableCell align="right" sx={{ fontSize: 12 }}>
                          {toVND(
                            Number(
                              checkoutInfo?.is_hourly
                                ? r.base_rate ?? 0 // ưu tiên base_rate (BE map hourly_rate), fallback rate
                                : r.base_rate ?? 0
                            )
                          )}
                        </TableCell>
                        <TableCell align="right" sx={{ fontSize: 12 }}>
                          {(() => {
                            const n =
                              Number(
                                // ưu tiên unit_count từ BE; fallback sang tổng hours/nights nếu có
                                (r as any)?.unit_count ??
                                  (checkoutInfo?.is_hourly
                                    ? checkoutInfo?.hours
                                    : checkoutInfo?.nights) ??
              ...
              <Paper variant="outlined" sx={{ p: 1.25, borderRadius: 2.5 }}>
                ...
                >
                  ...
                  <Button
                    ...
                    color="info"
                    disabled={
                      roomOptions.length === 0 || loadingOptions || !canAddMore
                    }
                    sx={{ textTransform: "none", borderRadius: 2 }}
                    title={
                      !canAddMore
                        ? "Không còn tiện nghi nào có thể thêm"
                        : undefined
                    }
                  >
                    Thêm
                ...
                >
                  ...
                  <TableBody>
                    ...
                    {amenitiesUsed?.map((row, idx) => {
                      ...
                      const currentRemaining = row.amenity_id
                        ? Math.max(
                            0,
                            ...
                            <TextField
                              ...
                              title={
                                currentRemaining !== undefined
                                  ? `Tối đa còn lại: ${currentRemaining}`
                                  : undefined
                              }
                            />
                          </TableCell>
