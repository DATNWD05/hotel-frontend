...
import {
  ...
  Button, Box, Paper, Typography, Divider, CircularProgress, Stack,
} from "@mui/material";
...
const getBookingStatus = (status: string): { status: string; color: string } => {
  switch ((status || "").toLowerCase()) {
    case "pending": return { status: "Chờ xác nhận", color: "#FFA500" };
    case "confirmed": return { status: "Đã xác nhận", color: "#388E3C" };
    case "checked-in": return { status: "Đã nhận phòng", color: "#1A73E8" };
    case "checked-out": return { status: "Đã trả phòng", color: "#757575" };
    case "canceled":
    case "cancelled": return { status: "Đã hủy", color: "#D32F2F" };
    default: return { status: "Không xác định", color: "#757575" };
  }
};
...
const InvoiceDialog: React.FC<InvoiceDialogProps> = ({
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle sx={{ fontWeight: 600, fontSize: "25px", color: "#0288d1", mb: 2, textAlign: "center" }}>
        🧾 HÓA ĐƠN THANH TOÁN
      </DialogTitle>
      ...
      <DialogContent dividers sx={{ px: 4, py: 3 }}>
        {invoiceLoading ? (
          ...
          <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
            {/* Header */}
            <Paper elevation={1} sx={{ p: 3, borderRadius: 3, border: "2px solid #0288d1", backgroundColor: "#e3f2fd", textAlign: "center" }}>
              <Typography variant="h5" fontWeight={700} sx={{ color: "#0288d1", mb: 1 }}>
                {vm.code}
              </Typography>
            ...
            {/* Thông tin đặt phòng */}
            <Paper elevation={1} sx={{ p: 2, borderRadius: 2, border: "1px solid #e0e0e0", backgroundColor: "#fafafa" }}>
              <Typography variant="h6" fontWeight={700} gutterBottom sx={{ color: "#333" }}>
                📋 Thông tin đặt phòng
              </Typography>
              <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 1 }}>
                <InfoRow label="Mã đặt phòng" value={`#${vm.bookingId}`} />
                <InfoRow label="Trạng thái" value={vm.statusText.status} valueColor={vm.statusText.color} />
                <InfoRow label="Check-in" value={vm.checkIn} />
                <InfoRow label="Check-out" value={vm.checkOut} />
                {vm.durationText && <InfoRow label="Hình thức tính" value={vm.durationText} />}
              </Box>
            </Paper>
            ...
            {vm.services.length > 0 && (
              <Paper elevation={1} sx={{ p: 2, borderRadius: 2, border: "1px solid #e0e0e0", backgroundColor: "#fafafa" }}>
                ...
                <Stack spacing={1}>
                  {vm.services.map((s) => (
                    <Line key={`srv-${s.service_id}`}
                      left={`${s.name} × ${s.quantity}`}
                      right={`${formatCurrency(s.price)} × ${s.quantity} = ${formatCurrency(s.total)}`}
                    />
                  ))}
                </Stack>
              </Paper>
              ...
              <Paper elevation={1} sx={{ p: 2, borderRadius: 2, border: "1px solid #e0e0e0", backgroundColor: "#fafafa" }}>
                ...
                <Stack spacing={1}>
                  {vm.amenities.map((a, idx) => (
                    <Line key={`amen-${a.amenity_id}-${idx}`}
                      left={`${a.amenity_name}${a.room_number ? ` (Phòng ${a.room_number})` : ""} × ${a.quantity}`}
                      right={`${formatCurrency(a.price)} × ${a.quantity} = ${formatCurrency(a.total)}`}
                    />
                  ))}
                </Stack>
              </Paper>
            ...
            <Paper elevation={1} sx={{ p: 2, borderRadius: 2, border: "1px solid #e0e0e0", backgroundColor: "#fafafa" }}>
              ...
              <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
                <Row label="Tiền phòng" value={formatCurrency(vm.amounts.room)} />
                <Row label="Tiền dịch vụ" value={formatCurrency(vm.amounts.service)} />
                <Row label="Tiền tiện nghi" value={formatCurrency(vm.amounts.amenity)} />
                <Row label="Giảm giá" value={Number(vm.amounts.discount) > 0 ? `-${formatCurrency(vm.amounts.discount)}` : formatCurrency(0)} mutedColor="#4caf50" />
                <Row label="Tiền đặt cọc" value={formatCurrency(vm.amounts.deposit)} />

                <Divider sx={{ my: 1 }} />
                <Box sx={{ display: "flex", justifyContent: "space-between", py: 2, backgroundColor: "#e3f2fd", px: 2, borderRadius: 2, border: "2px solid #0288d1" }}>
      ...
      <DialogActions sx={{ px: 3, pb: 2, justifyContent: "space-between" }}>
        <Button onClick={onClose} color="inherit" sx={{ fontWeight: 600, textTransform: "none", borderRadius: 2, fontSize: "15px" }}>
          Đóng
        </Button>
        <Button
          variant="contained"
          ...
          sx={{ borderRadius: 2, px: 3, fontWeight: 600, textTransform: "none", fontSize: "15px" }}
        >
...
const Row = ({ label, value, mutedColor }: { label: string; value: string; mutedColor?: string }) => (
